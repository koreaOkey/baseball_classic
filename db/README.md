...
- `public.games`: 경기 스냅샷 상태
  - 점수, 이닝, B/S/O, 주자 상태, 타자/투수, 상태(LIVE/SCHEDULED/FINISHED/CANCELED/POSTPONED)
...
## Migration Notes (2026-05-07, allow CANCELED/POSTPONED)

Added migration:

- `migrations/20260507_010_allow_canceled_postponed_game_status.sql`

`games.status` check 제약을 확장해 `'CANCELED'` 와 `'POSTPONED'` 를 허용한다.
백엔드/크롤러/앱은 이미 두 상태를 알고 있고, DB 제약만 막혀있던 상태였다.

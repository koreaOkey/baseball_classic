...
///
/// - 게이지는 0…1 범위로 한정되며 1.0 초과 불가.
/// - reset() 후에도 UserDefaults 기록은 유지된다 (Phase 2 광고 게이트 재사용).
/// - SwiftUI 바인딩이 필요하면 ObservableObject 래퍼(VentingRoomViewModel)를 사용한다.
public final class DestructionStateMachine: @unchecked Sendable {

    // MARK: - State

    /// 현재 게이지 값 (0.0 ~ 1.0).
    public private(set) var gauge: Double = 0.0

    /// 현재 파괴 단계.
    public private(set) var stage: DestructionStage = .idle
    ...
    public init(gameID: String, userDefaults: UserDefaults = .standard) {
        self.gameID = gameID
    ...
    @discardableResult
    public func tap() -> DestructionStage? {
    ...
    /// 게이지와 단계를 초기 상태(0.0, idle)로 되돌린다.
    ...
    public var hasRecordedFirstDestruction: Bool {

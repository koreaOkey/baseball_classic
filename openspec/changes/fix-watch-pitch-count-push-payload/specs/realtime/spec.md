...
#### Scenario: 워치 직접 push 수신 시 투구수 보존
- GIVEN 워치가 동일 game_id 에 대해 마지막으로 받은 `pitcherPitchCount = 16` 을 메모리에 가지고 있을 때
- WHEN 워치가 `pitcher_pitch_count` 키가 누락된 silent push 를 직접 수신하면
- THEN 워치는 이전 값 16 을 유지하고 투구수 영역을 계속 표시한다 (구버전 백엔드와의 호환)

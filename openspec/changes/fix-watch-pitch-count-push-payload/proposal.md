...
### iOS Watch (direct APNs path)

- `ios/watch/BaseHapticWatch/WatchAppDelegate.swift` `parseGameData(from:)`
  - 푸시 페이로드에서 `pitcher_pitch_count` 추출을 추가. 기존 코드는 키 자체를 dict 에 옮기지 않아 모든 워치 직접 push 가 투구수를 nil 로 덮어쓰고 있었다 (폰 백그라운드 시 사라짐 = 이 경로).
  - 키가 빠진 페이로드에서는 워치 메모리의 마지막 `gameData.pitcherPitchCount` 로 폴백 (동일 game_id 매칭).
  - 키가 있으면 그대로 전달, sentinel `-1` → nil 처리는 기존 `WatchConnectivityManager.handleGameData` 디코더가 담당.

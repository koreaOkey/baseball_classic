...
- **iOS 폰** (`ios/mobile/BaseHaptic/Models/LiveEvent.swift`, `Models/AtBatGroup.swift` (신규), `Screens/LiveGameScreen.swift`)
  ...
  - Debug 빌드는 스테이징 Railway 백엔드 URL과 스테이징 Supabase URL/publishable key를 주입할 수 있고, 미주입 시 백엔드는 로컬 테스트 백엔드로 폴백한다. Release 빌드는 승격된 스테이징 서버를 운영 백엔드로 사용한다.

- **iOS Watch**
  - WatchConnectivity로 받는 game_data/haptic_event 흐름은 유지한다.
  - watchOS Debug 빌드의 독립 폴링/토큰 등록 URL은 스테이징 Railway 백엔드 URL을 주입할 수 있고, 미주입 시 로컬 테스트 백엔드로 폴백한다. Release 빌드는 승격된 스테이징 서버를 운영 백엔드로 사용한다.
...
- **Android 폰** (`apps/mobile/.../model/LiveEvent.kt`, `model/AtBatGroup.kt` (신규), `ui/screens/LiveGameScreen.kt`)
  ...
  - Debug 빌드는 스테이징 Railway 백엔드와 스테이징 Supabase 프로젝트를 사용하고, Release 빌드는 승격된 스테이징 서버를 운영 백엔드로 사용하도록 Gradle buildType 별 `BuildConfig` 값을 분리한다.
...
- **테스트 인프라 분리**: 스테이징은 운영과 같은 Railway Backend/Crawler + Supabase + Redis 구성을 사용하되 별도 리소스와 별도 secret으로 격리한다. iOS/watchOS Debug 빌드는 스테이징 백엔드 URL을 주입할 수 있고, iOS Debug 빌드는 스테이징 Supabase URL/publishable key도 주입할 수 있다. Android Debug 빌드는 스테이징 백엔드와 스테이징 Supabase를 사용한다. Release 빌드는 승격된 스테이징 서버 `https://baseballclassic-production-4796.up.railway.app`를 운영 백엔드로 사용한다.

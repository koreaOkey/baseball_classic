...
- [x] 5.2c 키 복원 재배포 후 JWT 실패 로그 소멸 확인(21:16 KST 기준 APNs 오류 로그 0건)
- [x] 5.2d 성공 발송이 로그에 안 남던 사각지대 보완: 60초 집계 `[APNs-stats] ok/failed/permanent` 로그 + 테스트
- [ ] 5.2e 다음 경기일 `[APNs-stats] ok>0` 확인(0 이거나 `ES256 signing failed` 면 APNS_KEY_BASE64 재설정)
- [x] 5.3 9/3 취소 경기(20260903HTNC02026) 크롤러 종료 확인 — 재배포 후 해당 경기 crawl 로그 없음

## 6. 백엔드 · 기상 예보 상류 격리 (9/6 추가)

- [x] 6.1 온디맨드 타임아웃 (2s,3s)·프리워밍 (3s,12s) 분리, 실패 TTL 120s
- [x] 6.2 같은 키 조회 진행 중이면 대기 없이 stale/빈 응답(pile-on 방지), 마지막 성공 예보 6h 대체
- [x] 6.3 온디맨드 날씨 조회 전용 스레드 풀(6) — 기본 to_thread 풀 점유로 DB 로더 지연되는 경로 차단
- [x] 6.4 테스트 5건 + 기존 스위트 137 passed
- [x] 6.5 배포 후 `/games/{id}/weather` 499 소멸 확인 — 19:12 KST 이후 4xx/5xx 0건, 날씨 응답 ≤2.6s(상류 다운 중, items 빈 값)

...
- **기상 예보 상류 격리(9/6 추가)**: 기상청 API 가 11~25s 지연되자 앱의 날씨 요청이 전부 5초
  타임아웃(499)으로 끝나고 요청 스레드가 12초씩 묶였다. 온디맨드 타임아웃을 (2s,3s) 로 줄이고
  프리워밍만 (3s,12s) 를 쓴다. 진행 중 조회에 뒤따르는 요청은 기다리지 않고, 마지막 성공
  예보(6h)로 대체 응답한다. 온디맨드 조회는 전용 스레드 풀에서 돌려 다른 요청을 지연시키지 않는다.

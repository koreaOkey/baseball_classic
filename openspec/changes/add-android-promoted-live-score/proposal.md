...
promoted 노티는 커스텀 RemoteViews가 금지되므로(자격 상실), 유일한 자유 픽셀 영역인 largeIcon(~48dp)에 베이스 다이아몬드 + BSO 카운트를 비트맵으로 그린다. 실기기에서 합성 아이콘의 BSO 점 가독성이 부족할 가능성에 대비해 두 가지 아이콘 모드를 스위치로 전환할 수 있게 한다.
...
- `LiveScoreNotificationManager.post()` OS 분기:
  - **API 36+**: 커스텀 뷰 없이 BigTextStyle — 타이틀 "LG 3 : 2 두산", subText "7회말", 본문 "타자 오지환 · 투수 곽빈", `setLargeIcon(다이아몬드+BSO 비트맵)`, `setShortCriticalText("3:2")`, `setRequestPromotedOngoing(true)`.
  - **API 35 이하**: 기존 커스텀 RemoteViews 카드 그대로 (동작 변화 없음).
- 신규 `LiveScorePromotedIconRenderer`: 384px 캔버스에 아이콘 렌더링, 2개 모드:
  - `COMPOSITE`(기본): 다이아몬드(위) + B/S/O 점(아래) 합성.
  - `DIAMOND_ONLY`(폴백): 아이콘은 다이아몬드 전용, BSO는 본문에 이모지("B🟢🟢⚪ S🟡⚪ O🔴🔴")로 이동.
  - 전환: SharedPreferences `promoted_icon_mode` = `composite` | `diamond_only` (기본 composite). 실기기 검증 후 결정.

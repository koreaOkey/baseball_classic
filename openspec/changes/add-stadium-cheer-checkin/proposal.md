...
초기 계획은 다크 머지였지만, 실테스트를 위해 **2026-05-05 기준 iOS phone/watchOS + 백엔드 + DB + iOS 권한 매니페스트는 활성화 상태**로 전환했다. **2026-06-02 기준 iOS와 Android 모두 `내 팀` 탭 진입점은 활성화**한다. Android phone/Wear OS는 일부 화면·송신 코드가 들어가 있으며 geofence 실제 등록, 권한 매니페스트, Wear 수신 dispatch는 아직 다크/미완료 상태로 유지한다.
...
- **Android phone/Wear OS**: `내 팀` 화면 골격과 체크인 허브 UI, `sendCheerTrigger`/watch 수신 함수는 추가되어 있다. 2026-06-02 기준 Android phone `내 팀` 탭 진입점은 노출했고, 권한/지오펜싱 실제 등록/설정 토글 노출은 후속 Android 적용 범위로 남긴다.
...
- **신규 탭 추가**: 기존 하단 탭(`홈` / `상점` / `설정`) 옆에 **`내 팀`** 탭(가칭) 신설.
  ...
  - iOS와 Android 모두 `SHOW_MY_TEAM_TAB=true`로 탭 진입점을 노출한다.
...
- `my-team-tab`: 하단 탭 4번째 진입점("내 팀") + 탭 내부 콘텐츠 컨테이너. 1차 콘텐츠는 `team-checkin-ranking` 임베드, 향후 팀별 뉴스 등 추가 가능한 자리 확보. iOS와 Android 모두 탭 진입점 활성화.
...
- **신규 탭 ("내 팀")**: iOS와 Android phone 모두 4번째 탭으로 활성화

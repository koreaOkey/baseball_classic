...
이미 Android 는 `SHOW_STADIUM_CHEER_THEMES = false` 컴파일 상수로 가려져 있고 Android 설정 화면에는 "경기장 응원" 항목이 존재하지 않는다. 2026-06-11 요청에 따라 `내 팀` 탭도 iOS와 Android 모두 하단 네비게이션에서 숨기되, 코드 / 데이터 / 백엔드 흐름은 보존한다.
...
- iOS `BaseHapticApp.swift`: `private let SHOW_MY_TEAM_TAB = false` 로 하단 네비 "내 팀" 탭을 미노출한다.
- Android: `SHOW_STADIUM_CHEER_THEMES = false`는 유지한다. `SHOW_MY_TEAM_TAB=false` 로 하단 네비 "내 팀" 탭 진입점을 미노출한다.
- 백엔드 / 워치 / 데이터 모델 변경 없음.
...
- `mobile-ios:navigation`: 하단 네비게이션에서 "내 팀" 탭이 사라진다.
- `mobile-android:navigation`: 하단 네비게이션에서 "내 팀" 탭이 사라진다.
...
- `mobile-ios:my-team-tab`: 사용자 노출 표면 일시 제거. 화면과 도메인 코드는 유지하므로 차후 플래그만 다시 켜면 복귀 가능.
- `mobile-android:my-team-tab`: 사용자 노출 표면 일시 제거. 화면과 도메인 코드는 유지하므로 차후 플래그만 다시 켜면 복귀 가능.

...
- [x] `ios/mobile/BaseHaptic/BaseHapticApp.swift` — `private let SHOW_MY_TEAM_TAB = false` 로 하단 네비게이션의 "내 팀" 탭을 숨김 처리.
- [x] `apps/mobile/app/src/main/java/com/basehaptic/mobile/MainActivity.kt` — `private const val SHOW_MY_TEAM_TAB = false` 로 하단 네비게이션의 "내 팀" 탭을 숨김 처리.

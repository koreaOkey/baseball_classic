...
- 사용자별 `preferred_event_types` 멀티 셀렉트 신설 — 어떤 이벤트(SCORE/HOMERUN/HIT/WALK/STEAL 등)를 long-look expand로 받을지 선택. 백엔드 저장 + 모바일 설정 UI + 워치 sync.
- **워치 우선 햅틱 정책**: 워치 페어링 + 활성 상태면 폰 햅틱·푸시 suppress. 한 이벤트 = 한 채널만 발화.
- 이벤트 필터는 **long-look expand 트리거에만 적용**. ongoing 스코어 갱신은 필터와 무관하게 항상 동작.
...
- ongoing 영역(항상 진행 중)은 설정 UI에 노출하지 않음. 설정에는 "선택한 이벤트만 알림" 한 줄로만 표시.
...
### New Capabilities
- `live-score-notification`: 폰/워치 양쪽의 라이브 스코어 ongoing 노티 + 선택 이벤트 long-look expand 정책·payload·suppress 룰 정의

### Modified Capabilities
- `mobile-android`: 폰 라이브 스코어 ongoing notification 신설, 워치 우선 햅틱 suppress 가드 추가
- `mobile-ios`: 사용자 설정 UI에 `preferred_event_types` 멀티 셀렉트 추가, 워치 우선 햅틱 suppress 가드 추가
...
- `realtime`: push payload에 `preferred_event_types` 적용 (장기적으로는 클라이언트 가드 우선)

## Impact

- **백엔드**: 사용자별 `preferred_event_types` 컬럼 신설(Supabase user preferences), push payload에 이벤트 타입 포함(이미 있을 경우 재활용), 큐잉 로직 제거(있다면)
- **iOS 폰**: 설정 화면에 이벤트 멀티 셀렉트 추가, 워치 페어링·활성 상태 감지해 햅틱·푸시 suppress 가드 추가
- **Android 폰**: 설정 화면에 이벤트 멀티 셀렉트 추가, Live Activity 대응 ongoing notification 신설, 워치 우선 햅틱 suppress 가드 추가

...
### Requirement: 잠금화면 경기 카드 토글
경기 LIVE 상태인 동안 폰에는 라이브 스코어 표시 채널(iOS Live Activity 또는 Android ongoing notification)을 게시해야 한다(MUST). 사용자는 모바일 설정의 "잠금화면 경기 카드" 토글로 이 채널을 제어할 수 있어야 하며, 기본값은 ON이어야 한다(MUST).

#### Scenario: 토글 ON 상태에서 LIVE 전환 시 노티 게시
- **GIVEN** 사용자가 "잠금화면 경기 카드"를 켠 상태에서
...
한 이벤트로 인한 햅틱은 워치·폰 중 한 곳에서만 발화되어야 한다(MUST). 워치가 페어링되어 있고 활성 상태(reachable 또는 동등 신호)이면 워치에서만 햅틱을 발화하고 폰은 햅틱·헤드업 알림을 발화하지 않는다. 다만 폰의 라이브 스코어 노티/Live Activity 갱신은 "잠금화면 경기 카드"가 켜진 경우 정상 동작한다.
...
워치가 비활성/도달 불가 상태로 판단되는 경우(시나리오 3·6) 폰도 햅틱·헤드업 노티를 발화하지 않아야 한다(MUST). 단, "잠금화면 경기 카드"가 켜져 있으면 폰 라이브 스코어 노티/Live Activity 갱신은 정상 동작한다.

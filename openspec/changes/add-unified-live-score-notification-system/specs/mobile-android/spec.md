...
#### Scenario: 잠금화면 토글 ON 알림 권한 차단
- **GIVEN** 앱 알림 권한이 꺼져 있는 상태에서
- **WHEN** 사용자가 오늘의 경기 카드 하단의 "잠금화면" 토글을 ON으로 전환하면
- **THEN** live_score ongoing notification을 시작하지 않고 "경기 확인을 위해서는 알림 허용이 필요합니다." 안내를 표시한다
- **AND** 사용자가 [알림 허용] 을 탭하면 알림 권한 요청 또는 알림 설정 화면으로 이동한다

...
Android 폰은 라이브 경기 동안 잠금화면·알림 드로어에서 확인 가능한 라이브 스코어 ongoing notification을 게시해야 한다(MUST). 이 기능은 iOS의 "잠금화면 경기 카드"와 동일한 설정 토글로 제어되며 기본값은 ON이어야 한다(MUST). 동일 식별자를 사용해 스코어·이닝·BSO·최근 이벤트 1개를 in-place 갱신한다.

#### Scenario: LIVE 동안 노티 노출
- **GIVEN** 사용자 관심 경기가 LIVE 상태이고 "잠금화면 경기 카드"가 켜져 있을 때
...
워치(Wear OS) 노드가 페어링·연결된 상태에서는 폰 햅틱·헤드업 노티를 발화하지 않아야 한다(MUST). 단, "잠금화면 경기 카드"가 켜져 있으면 라이브 스코어 ongoing notification 갱신은 정상 동작한다.
...
오늘의 경기 카드 하단에 "잠금화면" 토글과 "Watch" 토글을 노출하고, 설정 화면에도 "잠금화면 경기 카드" 토글과 "선택한 이벤트만 알림" 항목을 노출해야 한다(MUST). "잠금화면" 및 "잠금화면 경기 카드"는 Android live_score ongoing notification 표시 여부를 제어하고, "Watch"는 워치 동기화 여부를 제어한다. "선택한 이벤트만 알림"은 SCORE·HOMERUN·HIT·WALK·STEAL 등 강한 이벤트 알림 타입을 멀티 셀렉트로 지정한다.

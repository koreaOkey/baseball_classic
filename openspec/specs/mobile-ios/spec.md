# Mobile iOS Specification

## Purpose
iOS(SwiftUI) 아이폰에서 실시간 야구 경기를 확인하고 Apple Watch와 연동하는 모바일 앱.
## Requirements
...
- THEN Apple Watch에 동기화 프롬프트를 전송한다

### Requirement: Apple Watch 동기화 플로우
모바일은 Apple Watch와 WatchConnectivity 프레임워크를 통해 양방향 동기화해야 한다(MUST).

#### Scenario: 워치 동기화 요청
- GIVEN 관심 팀 경기가 LIVE가 되었을 때
- WHEN 모바일이 WCSession을 통해 동기화 프롬프트를 전송하면
- THEN Apple Watch에 동기화 동의 팝업이 표시된다
...
- THEN 스테이징 Supabase 주소와 publishable key가 제공된 경우 해당 프로젝트를 사용한다
- AND Release 빌드는 운영 Supabase 프로젝트를 사용한다

### Requirement: 백그라운드 앱 새로고침
iOS 백그라운드 제약 내에서 경기 데이터를 유지해야 한다(MUST).

#### Scenario: 백그라운드 전환 시 데이터 유지
- GIVEN 사용자가 앱을 백그라운드로 전환했을 때
- WHEN 경기가 진행 중이면
...
- **THEN** 워치 동기화 alert 를 표시하지 않고 LiveGame으로 바로 이동한다
...
iOS 모바일 앱의 라이브 경기 필드 카드는 백엔드 `GameStateOut.homeLineup` / `awayLineup` 을 수비팀 기준으로 매핑하여 8 개 수비 슬롯 라벨을 자동 표시해야 한다(MUST).
...
#### Scenario: 이닝 탭 지연 로드
- **GIVEN** 라이브 경기 상세 화면이 현재 회 이벤트만 먼저 표시한 상태일 때
- **WHEN** 사용자가 아직 불러오지 않은 회 탭을 탭하면
- **THEN** 앱은 해당 회 이벤트를 백엔드에서 조회하여 표시하고 이후 같은 회 탭 재진입 시 캐시된 이벤트를 사용한다

#### Scenario: 득점 탭 지연 로드
- **GIVEN** 라이브 경기 상세 화면에서 득점 탭 이벤트를 아직 불러오지 않은 상태일 때
- **WHEN** 사용자가 득점 탭을 탭하면
- **THEN** 앱은 득점 이벤트만 백엔드에서 조회하여 표시하고 이후 같은 탭 재진입 시 캐시된 이벤트를 사용한다
...
### Requirement: 라이브 중계 타석 상세 필드 수신
iOS 모바일 앱은 백엔드 이벤트 응답에 포함된 투구 상세 필드를 optional 데이터로 수신해야 한다(MUST).

#### Scenario: 투구 상세 필드 수신
- **GIVEN** 백엔드 이벤트 응답에 구속, 구종, 투구 번호, 투구 후 볼/스트라이크/아웃 카운트, 타자 기록, 승리확률 정보가 포함되어 있을 때
- **WHEN** 앱이 라이브 이벤트를 파싱하면
- **THEN** 해당 값을 이벤트 모델에 보존하여 타석 카드 UI에서 사용할 수 있다

#### Scenario: 투구 상세 필드 미제공 폴백
- **GIVEN** 백엔드 이벤트 응답에 투구 상세 필드가 없을 때
- **WHEN** 앱이 라이브 이벤트를 파싱하면
- **THEN** 기존 이벤트 타입, 설명, 타자, 투수, 이닝, 타석 그룹 키만으로 화면을 표시한다

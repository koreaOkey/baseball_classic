...
#### Scenario: 투구 상세 메타데이터 보존
- GIVEN 릴레이 투구 이벤트에 구속, 구종, 투구 번호, 투구 후 볼/스트라이크/아웃 카운트, 타자 기록, 승리확률 정보가 포함되어 있을 때
- WHEN 크롤러가 백엔드 인제스트 이벤트를 생성하면
- THEN 해당 값들을 이벤트 메타데이터에 보존하여 모바일 상세 화면이 타석 카드로 재구성할 수 있게 한다
